...
static void SNDsaveWave(char *file, char *buffer, int32_t size,
                        int32_t speed)
{
    FILE *fp;
    int32_t wsize, sz = 0;
    ...
    write_i32_le(&headr.length,
                 4 + sizeof(struct WAVEFMT) + sizeof(struct WAVEDATA) +
                 size);
...
char *SNDloadWaveFile(char *file, int32_t * psize, int32_t * pspeed)
{
    ...
    if (strncmp(headr.riff, "RIFF", 4) != 0)
        ProgError("RR12", "%s: bad RIFF magic in header (%s)", fname(file),
                  short_dump(headr.riff, 4));
    if (strncmp(headr.wave, "WAVE", 4) != 0)
        ProgError("RR13", "%s: bad WAVE magic in header (%s)", fname(file),
                  short_dump(headr.wave, 4));
    ...
    for (sz = 0;; sz++) {
        ...
        if (strncmp(headc.name, "data", 4) == 0)
            break;
    ...
    if (datasize > 0x1000000L)  /* AYM 2000-04-22 */
        ProgError("RR21", "%s: sample too long (%ld)", fname(file),
                  (long) datasize);
    /*read data */
    data = (char *) Malloc(datasize);
    for (wsize = 0; wsize < datasize; wsize += sz) {
        sz = (datasize - wsize >
              MEMORYCACHE) ? MEMORYCACHE : (datasize - wsize);
                  ...
                  const char *name)
{
    ...
    if (datasize > phys_size) {
        Warning("SD12",
                ...
                lump_name(name), (unsigned long) datasize,
...
void SNDsavePCSound(const char *lumpname, const char *file,
