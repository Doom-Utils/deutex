...
void PNMinit(char *buffer, int32_t size)
{
    ...
    if (size > 4L) {
        ...
        if (pnames > 0x7FFFL)
            ProgError("PA01", "Too many patches");
        if (size < (4L + 8L * pnames))
            ProgError("PA02", "PNAMES has wrong size");
    }
    /*initialise */
...
static int16_t PNMgetPatchIndex(char *patch)
{
    ...
    if (idx < 0) {              /*No, it's a new patch, then */
        ...
        if (PNMtop >= PNMmax) {
            PNMmax += NEWPATCHS;
            PNMpatchs =
                (struct PNMP *) Realloc(PNMpatchs,
                                        PNMmax * sizeof(struct PNMP));
        }
    }
    return idx;
}
...
static void TXUdefineCurTex(char name[8], int16_t X, int16_t Y,
                            bool Redefn)
{
    int t;
    if (!TXUok)
    ...
    if (TXUtexTop >= TXUtexMax) {
        TXUtexMax += NEWTEXTURES;
        TXUtex =
            (struct TEXTUR *) Realloc(TXUtex,
                                      TXUtexMax * sizeof(struct TEXTUR));
    }
...
static void TXUaddPatchToCurTex(int16_t pindex, int16_t ofsX, int16_t ofsY)
{
    ...
    if (TXUpatTop >= TXUpatMax) {
        TXUpatMax += NEWPATCHESDEF;
        TXUpat =
            (struct PATCH *) Realloc(TXUpat,
                                     TXUpatMax * sizeof(struct PATCH));
    }
    if (TXUtexCur < 0)
        Bug("TX92", "TXUTxC");
...
static int32_t TXUrealTexture(void)
{
    ...
    for (t = 0; t < TXUtexTop; t++) {
        if (TXUtex[t].Npatches < 1) {
            Warning("TX10", "Ignored empty texture %s",
    ...
    for (pat = 0, tt = 0, t = 0; t < TXUtexTop; t++) {
        if (TXUtex[t].Name[0] != '\0') {
            ...
            if (output_texture_format != TF_NAMELESS) {
                wad_write_name(info->fd, TXUtex[t].Name);
                size += 8;
            }
            wad_write_i16(info->fd, 0);
            size += 2;
            wad_write_i16(info->fd, 0);
            size += 2;
            wad_write_i16(info->fd, TXUtex[t].szX);
            size += 2;
            wad_write_i16(info->fd, TXUtex[t].szY);
            size += 2;
            ...
            wad_write_i16(info->fd, TXUtex[t].Npatches);
            size += 2;
            for (p = 0; p < (TXUtex[t].Npatches); p++) {
                if (pat + p >= TXUpatTop)
                    /*number of patches exceeds definitions */
                    Bug("TX98", "TxuP>D");
                wad_write_i16(info->fd, TXUpat[pat + p].ofsX);
                size += 2;
                wad_write_i16(info->fd, TXUpat[pat + p].ofsY);
                size += 2;
                wad_write_i16(info->fd, TXUpat[pat + p].Pindex);
                size += 2;
                if (output_texture_format != TF_STRIFE11) {
                    wad_write_i16(info->fd, 0);
                    size += 2;
                    ...
                    bool Redefn)
{
    ...
    if (PatchSz > 0) {
        ...
        if (dummy > PatchSz)
            ProgError("PA03", "PNAMES is corrupt");
    } else {
        MaxPindex = PNMgetNbOfPatch();
    }
    if ((MaxPindex < 0) || (MaxPindex > 0x7FFF))
        ProgError("PA04", "PNAMES: invalid patch count %ld",
                  (long) MaxPindex);
    ...
    for (t = 0; t < Numtex; t++) {
        ...
        if (have_texture_name) {
            ...
            if (t > 9999) {
                Warning("TX15",
                        "%s: more than 10000 textures, ignoring excess",
        ...
        if ((Xsize < 0) || (Xsize > 4096))
            ProgError("TX16", "%s: Texture %s: width out of bounds (%d)",
                      lump_name(texture1_name), lump_name(tname),
                      (int) Xsize);
        ...
        for (p = 0; p < Numpat; p++, Pos += item_size) {
            ...
            if ((Xofs < -4096) || (Xofs > 4096))
                ProgError("TX20",
                          "%s(%ld): Texture %s(%d/%d): bad patch X-offset %d",
                          lump_name(texture1_name), (long) t,
                          lump_name(tname), (int) p, (int) Numpat,
                          (int) Xofs);
            Yofs = peek_i16_le(Data + Pos + 2);
            if ((Yofs < -4096) || (Yofs > 4096))
                ProgError("TX21",
                          "%s(%ld): Texture %s(%d/%d): bad patch Y-offset %d",
                          lump_name(texture1_name), (long) t,
                          lump_name(tname), (int) p, (int) Numpat,
                          (int) Yofs);
            ...
            if ((Pindex < 0) || (Pindex > MaxPindex))
                ProgError("TX22",
                          "%s(%ld): Texture %s(%d/%d): bad patch index %d",
                          lump_name(texture1_name), (long) t,
                          lump_name(tname), (int) p, (int) Numpat,
                          (int) Pindex);
...
bool TXUcheckTex(int16_t npatch, int16_t * PszX)
{
    ...
    if (TXUtexTop < 100)
        Output
            ("Warning: Some textures could be missing! (less than 100 defined)\n");
    for (pat = 0, t = 0; t < TXUtexTop; t++) {
        ...
        ** check width
        */
        for (bit = 1, C = 0, b = 0; b < 16; b++, bit <<= 1)
            if ((TXUtex[t].szX) & bit)
                C++;
        if (C > 1) {
            /* FIXME assign a code to this message */
        ...
        */
        ...
        for (col = 0; col < TXUtex[t].szX; col++) {
            if (Meduza < 2)
                Meduza = 0;     /*no Meduza effect found yet */
            found = false;
            for (p = 0; p < (TXUtex[t].Npatches); p++) {
                ...
                if (col >= TXUpat[pat + p].ofsX) {
                    top =
                        PszX[TXUpat[pat + p].Pindex] + TXUpat[pat +
                                                              p].ofsX;
                    if (col < top) {
                        found = true;
                        if (Meduza >= 2)
        ...
        if (Meduza >= 2) {
            /*there is a column with two patches */
            /* FIXME assign a code to this message */
            Output
                ("Warning: Texture %s should not be used on a two sided wall.\n",
                 lump_name(TXUtex[t].Name));
        }
        pat += TXUtex[t].Npatches;
    }
...
void TXUwriteTexFile(const char *file)
{
    ...
    if (out == NULL)
        ProgError("TX31", "%s: %s", file, strerror(errno));
    TXUrealTexture();
...
void TXUreadTexFile(const char *file, bool Redefn)
{
    ...
    for (t = 0; t < MAXTEXTURES; t++) {
        ...
        /* check X size */
        if (xsize < 0)
            ProgError("TX35", "Texture %s: width %d < 0", lump_name(tname),
                      (int) xsize);
        if (xsize > 4096)
            Warning("TX36",
        ...
        if (xsize > 512)
            Warning("TX37", "Texture %s: width %d > 512, Doom may freeze",
                    lump_name(tname), (int) xsize);
        for (bit = 1, C = 0, b = 0; b < 16; b++, bit <<= 1)
            if (xsize & bit)
                C++;
        ...
        TXUdefineCurTex(tname, xsize, ysize, Redefn);
        {
            ...
            while (TXTreadPatchDef(TXT, pname, &ofsx, &ofsy)) {
                ...
                else if (npat > MAX_PATCHES);   /* Silently drop the rest */
                else {
                    /* declare a patch wall, getting index or
